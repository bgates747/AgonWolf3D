...
> Build-layout update, 2026-07-25: generated metadata for every final-assembly
> asset family now occupies an independently marked section of
> `src/asm/autogenerated.asm`. References below to the former standalone
> generated files describe the integration history.
...
src/asm/autogenerated.asm generated final-assembly metadata, grouped by section
...
91  assign image buffer IDs and replace the IMAGES assembly section
...
## Stage 91: current assembly catalog
...
and replaces the `IMAGES` section in `src/asm/autogenerated.asm`.
...
`src/asm/wolf3d.asm` includes the single generated
`src/asm/autogenerated.asm`. During `init`, it
...
1. the `BUF_*` constants and runtime buffer-ID lookup tables emitted to
   the `IMAGES` section of `src/asm/autogenerated.asm`; and
...
assignment. After replacing the `IMAGES` section, stage 91 parses
`images.agnb` and requires all ordered cube, sprite, and distance-wall `BHDR`
IDs to match the shared catalog before final assembly can proceed.
...
### Hook 4: generated assembly section

The `IMAGES` section of `src/asm/autogenerated.asm` contains:
