...
def delete_temp_files(directory):
    ...
    patterns_to_delete = ['*.cmb.*', 'temp.asm']
...
if __name__ == "__main__":
    ...
    src_filenames = ['wolf3d.asm', 'wolf3d_init.asm', 'wolf3d_main.asm']
    src_base_dir = 'src/asm/'
    tgt_base_dir = 'tgt/'
    # moslet_dir = '/home/smith/Agon/emulator/v0.9.26src/sdcard/mos/'
    moslet_dir = None
    moslet_bin = None
    if moslet_dir is not None:
        moslet_bin = src_filenames[0].split('.')[0] + ".bin"
...
# PASS ONE ASSEMBLY
    ...
    # PASS TWO ASSEMBLY
        ...
        if assembled == 0:
            ...
            if moslet_bin is not None:
                moslet_bin_src_filepath = os.path.join(tgt_base_dir, moslet_bin)
                moslet_bin_tgt_filepath = os.path.join(moslet_dir, moslet_bin)
                print(f"Copying {moslet_bin_src_filepath} to {moslet_bin_tgt_filepath}")
                shutil.copy(moslet_bin_src_filepath, moslet_bin_tgt_filepath)
