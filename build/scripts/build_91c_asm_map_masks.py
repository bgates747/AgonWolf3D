...
def update_maps_asm(db_path):
    asm_file_path = os.path.join('src', 'asm', 'maps.asm')

    # Read the existing content of the asm file up to the autogenerated comment line
    with open(asm_file_path, 'r') as asm_file:
        asm_content = asm_file.readlines()

    auto_generated_section_start = None
    for i, line in enumerate(asm_content):
        if "; #### AUTO-GENERATED MAP DATA BELOW THIS LINE DO NOT EDIT ####" in line:
            auto_generated_section_start = i + 1
            break

    if auto_generated_section_start is None:
        raise ValueError("Could not find the autogenerated section in maps.asm")

    # Trim the content to only include lines above the autogenerated section
    asm_content = asm_content[:auto_generated_section_start]

    # Fetch floor and room data from the database
    conn = sqlite3.connect(db_path)
    conn.row_factory = sqlite3.Row
    cursor = conn.cursor()
    cursor.execute("SELECT DISTINCT floor_num, room_id FROM tbl_06_maps ORDER BY floor_num, room_id")
    rows = cursor.fetchall()
    # Get the obj_id of the start cell
    cursor.execute("""SELECT obj_id FROM tbl_02_tiles WHERE special = 'start' AND is_active = 1""")    
    start_obj_id = cursor.fetchone()[0]
    conn.close()

    # Write start_obj_id to the asm file
    asm_content.append(f"\nstart_obj_id: equ {start_obj_id}\n")
    ...
    for row in rows:
        ...
        if floor_num not in floors:
            floors[floor_num] = []
        floors[floor_num].append(room_id)

    # Build the new content
    asm_content.append("\nfloors:\n")
    for floor_num in sorted(floors.keys()):
        asm_content.append(f"\tdl floor_{floor_num:02d}\n")

    asm_content.append("\nroom_files:\n")
    for floor_num in sorted(floors.keys()):
        asm_content.append(f"floor_{floor_num:02d}:\n")
        for room_id in sorted(floors[floor_num]):
            asm_content.append(f"\tdl room_{floor_num:02d}_{room_id}\n")

    asm_content.append("\n")
    for floor_num in sorted(floors.keys()):
        for room_id in sorted(floors[floor_num]):
            asm_content.append(f"room_{floor_num:02d}_{room_id}: db \"maps/map_{floor_num:02d}_{room_id}.bin\",0\n")

    # Write the new content to the asm file
    with open(asm_file_path, 'w') as asm_file:
        asm_file.write("".join(asm_content))
