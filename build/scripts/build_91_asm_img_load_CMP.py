...
def make_asm_img_load(db_path, panels_inc_path, render_type, src_img_dir, next_buffer_id_counter, unique_rows):
    ...
    with open(panels_inc_path, 'a') as asm_writer: # append to the file
        ...
        # compress the source image file
            src_file = f"tgt/{src_img_dir}/{panel_base_filename}.rgba2"
            cmp_filepath = src_file + ".cmp"
            os.system(f"./compress {src_file} {cmp_filepath}")

        # get the filesize of the compressed image
            cmp_filesize = os.path.getsize(cmp_filepath)

        # write the assembly code to load the image
            asm_writer.write(f"\n")
            ...
            asm_writer.write(f"\tld ix,{cmp_filesize}\n")
            asm_writer.write("\tcall vdu_load_img_cmp\n")
            asm_writer.write("\tret\n")

        asm_writer.write("\n; File name lookups:\n")
        for row in unique_rows:
            panel_base_filename = row['panel_base_filename']
            asm_writer.write(f"F{panel_base_filename}: db \"{src_img_dir}/{panel_base_filename}.rgba2.cmp\",0\n")
...
if __name__ == "__main__":
    ...
    panels_inc_path = "src/asm/images_CMP.asm"
