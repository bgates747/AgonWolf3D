...
def make_asm_img_load(
    db_path,
    panels_inc_path,
    render_type,
    src_img_dir,
    next_buffer_id_counter,
    unique_rows,
...
def make_asm_images_inc(db_path, panels_inc_path, next_buffer_id_counter):
    ...
    with open(panels_inc_path, 'w') as asm_writer:
        asm_writer.write("; This file is created by build_91_asm_images.py, do not edit it!\n")

    # generate the code for the 3D panels
    render_type = "cube"
    src_img_dir = "panels"
    unique_rows = get_panels_data(db_path, render_type)
    next_buffer_id_counter = make_asm_img_load(
        db_path,
        panels_inc_path,
        render_type,
        src_img_dir,
