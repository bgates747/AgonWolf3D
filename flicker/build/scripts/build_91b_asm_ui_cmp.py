...
def make_rgba2_files(db_path, src_png_dir, tgt_cmp_rgba2_dir):
    # create target directory if it doesn't exist
    if not os.path.exists(tgt_cmp_rgba2_dir):
        os.makedirs(tgt_cmp_rgba2_dir)

    # delete all .rgba2 files in target dir
    for f in os.listdir(tgt_cmp_rgba2_dir):
        if f.endswith('.rgba2'):
            os.remove(os.path.join(tgt_cmp_rgba2_dir, f))
    ...
    for row in img_rows:
        ...
        tgt_cmp_file = os.path.join(tgt_cmp_rgba2_dir, panel_base_filename + ".rgba2")
        src_img = Image.open(src_file)
        img_to_rgba2(src_img, tgt_cmp_file)
    conn.close()
